...
public class KeysController : ControllerBase
{
    ...
    public async Task<ActionResult<KeyBundleDto>> UploadBundle([FromBody] UploadKeyBundleRequest request)
    {
        ...
        if (bundle is null)
        {
            bundle = new UserKeyBundle { UserId = userId.Value, IdentityPublicKey = request.IdentityPublicKey.Trim(), UpdatedAt = DateTime.UtcNow };
    ...
    public async Task<ActionResult<object>> Me()
    {
        ...
        return Ok(new { hasKeys = bundle is not null, updatedAt = bundle?.UpdatedAt });
    }
}
